...
function AtlasPage() {
  ...
  // Daily-refreshed area intel: weather + news at route midpoint. Wildlife is
  // fetched separately and lazily (see below) — its lookup is expensive (a
  // facet search plus up to 10 sequential per-species detail calls) and most
  // visits never open that panel, so there's no reason to pay for it upfront.
  useEffect(() => {
    if (!start || !end) {
      setWeather(null);
      setNews([]);
    ...
    Promise.all([fetchWeather(mid), fetchNews(mid, placeName)]).then(([w, n]) => {
      if (cancelled) return;
      setWeather(w);
      setNews(n);
    ...
    <main className="min-h-screen" style={{ background: "var(--paper-deep)" }}>
      ...
      <section className="mx-auto max-w-[1600px] px-4 py-6">
        ...
        <div
          ...
          style={{ height: "min(65vh, 700px)", width: "100%", zIndex: 0 }}
        />
